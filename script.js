...
function choose(year) {
	...
	data.then(function(ele) {
		...
		let exp_mass_shooting = ["all", ["==", "date", parseInt(year)]];
		...
		map.on("load", function() {
			map.addLayer({
				...
				paint: {
					...
					"circle-radius": [
						"+",
						["*", ["/", ["-", ["get", "total_victims"], min_fa], dif_fa], 35],
						10
					],
	...
	map.on("click", function(e) {
		...
		features.forEach(function(feature) {
			if (feature.layer.id === "mass_shooting") clicked = feature;
		});
		...
		description +=
			"<strong>Total Victims:</strong> " + total + "<" + "br" + ">";
		description +=
			"<strong>Weapon Legality:</strong> " + legality + "<" + "br" + ">";
		...
		map.on("load", function() {
			...
			huntingPromise.then(function(huntingData) {
				...
				const getMinMaxVal = (rangeMin, rangeMax, curVal) => {
					return (
						((rangeMax - rangeMin) / (max - min)) * (curVal - max) + rangeMax
					);
				};
				...
				$.each(data, (state, val) => {
					...
					const height = Math.ceil(
						getMinMaxVal(20, 90, val["huntingperperson"])
					);
					...
					features.push({
						...
						properties: {
							...
							huntingProportion: `${(100 * val["huntingperperson"]).toFixed(
								2
							)}%`
						},
						geometry: {
				...
				function showDetail(location, layer, fields) {
					...
					if (
						features !== null &&
						features[0]["layer"]["id"] === "hunting_bars"
					) {
						var popupText = "";

						popupText = `<strong>${fields[0]}</strong>${
							features[0].properties["state"]
						}<br><strong>${fields[1]}</strong>${
							features[0].properties["statePop"]
						}<br><strong>${fields[2]}</strong>${
							features[0].properties["huntingPop"]
						}<br><strong>${fields[3]}</strong>${
							features[0].properties["huntingProportion"]
						}<br>`;
